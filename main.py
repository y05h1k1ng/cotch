...
def search_open_info(store):
    ...
    if r.json()['results'][0].get('opening_hours', False):
        return r.json()['results'][0]['opening_hours'].get('open_now', False)
    return r.json()['results'][0].get('opening_hours', False)
...
def dist(origin, destinations):
    ...
    request += "&mode=walking&key="+api_key
...
def search_store(longitude, latitude, start, keyword):
    ...
    payload = {'key':API_KEY, 'lng':longitude, 'lat':latitude, 'start':start, 'count': 10, 'keyword':keyword}
    ...
    for data in datas['results']['shop']:
        ...
        try:
            ...
            pass
        result = {'id': id, 'name':name, 'address':address, 'location': {'longitude':lng, 'latitude':lat}, 'budget':budget, 'catch':catch, 'images':images}
...
def make_store(store_info, prefecture, category_name):
    store_info['link'] = 'https://www.google.com/maps/?q={},{}'.format(store_info['location']['latitude'], store_info['location']['longitude'])
    store_info['is_open'] = search_open_info(store_info)
